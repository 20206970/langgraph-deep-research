...
class SQLiteRepository:
    ...
    def _initialize(self) -> None:
        with self._connection() as connection:
            ...
            self._ensure_column(connection, "plans", "owner_id", "TEXT NOT NULL DEFAULT ''")
            self._ensure_column(connection, "runs", "owner_id", "TEXT NOT NULL DEFAULT ''")
            self._ensure_column(connection, "task_runs", "owner_id", "TEXT NOT NULL DEFAULT ''")
            self._ensure_column(connection, "report_versions", "owner_id", "TEXT NOT NULL DEFAULT ''")
            self._ensure_column(connection, "event_logs", "owner_id", "TEXT NOT NULL DEFAULT ''")
            connection.executescript(
                """
                CREATE INDEX IF NOT EXISTS idx_plans_owner_created ON plans(owner_id, created_at DESC);
                CREATE INDEX IF NOT EXISTS idx_runs_owner_created ON runs(owner_id, created_at DESC);
                CREATE INDEX IF NOT EXISTS idx_task_runs_owner_run ON task_runs(owner_id, run_id);
                CREATE INDEX IF NOT EXISTS idx_reports_owner_run ON report_versions(owner_id, run_id);
                CREATE INDEX IF NOT EXISTS idx_events_owner_run ON event_logs(owner_id, run_id, event_id);
                """
            )
    ...
    def create_user(self, username: str, password_hash: str) -> dict[str, Any]:
        """Create a unique application user without exposing password hashes in responses."""
        ...
        try:
            ...
            raise InvalidStateTransitionError("username already exists") from error
        return {key: value for key, value in user.items() if key != "password_hash"}

    def get_user(self, user_id: str) -> dict[str, Any] | None:
        with self._connection() as connection:
            row = connection.execute(
                "SELECT user_id, username, password_hash, created_at FROM users WHERE user_id = ?", (user_id,)
            ).fetchone()
    ...
    def get_user_by_username(self, username: str) -> dict[str, Any] | None:
        with self._connection() as connection:
            row = connection.execute(
                "SELECT user_id, username, password_hash, created_at FROM users WHERE username = ? COLLATE NOCASE",
                (username,),
            ).fetchone()
        return dict(row) if row is not None else None
    ...
    def _event(
        ...
        *,
        ...
        connection.execute(
            "INSERT INTO event_logs(run_id, task_id, owner_id, event_type, payload_json, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            (run_id, task_id, owner_id, event_type, self._json(payload), utc_now()),
        )

    @staticmethod
    def _owner_for_run(connection: sqlite3.Connection, run_id: str) -> str:
        row = connection.execute("SELECT owner_id FROM runs WHERE run_id = ?", (run_id,)).fetchone()
        return str(row["owner_id"]) if row is not None else ""
        ...
        with self._connection() as connection:
            connection.execute(
                ...
                (
                    event.run_id,
                    event.task_id,
                    self._owner_for_run(connection, event.run_id),
                    event.type.value,
                    self._json(payload),
                    event.timestamp,
                ),
    ...
    def list_events(self, run_id: str, *, task_id: str | None = None, owner_id: str = "") -> list[ResearchEvent]:
        """Read structured events in insertion order; legacy rows are ignored."""
        with self._connection() as connection:
            if task_id is None:
                rows = connection.execute(
                    "SELECT payload_json FROM event_logs WHERE run_id = ? AND owner_id = ? ORDER BY event_id",
                    (run_id, owner_id),
                ).fetchall()
            else:
                rows = connection.execute(
                    "SELECT payload_json FROM event_logs WHERE run_id = ? AND task_id = ? AND owner_id = ? ORDER BY event_id",
    ...
    def create_plan(self, plan: TaskPlan, *, owner_id: str = "") -> dict[str, Any]:
        ...
        with self._connection() as connection:
            connection.execute(
                "INSERT INTO plans(plan_id, plan_version, owner_id, topic, payload_json, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (plan.plan_id, plan.plan_version, owner_id, plan.topic, self._json(payload), RunStatus.PLANNED.value, created_at),
            )
            self._event(
                connection,
                "plan_created",
        ...
        return self.get_plan(plan.plan_id, plan.plan_version, owner_id=owner_id)

    def get_plan(self, plan_id: str, plan_version: int, *, owner_id: str = "") -> dict[str, Any]:
        with self._connection() as connection:
            row = connection.execute(
                "SELECT payload_json, status, created_at FROM plans WHERE plan_id = ? AND plan_version = ? AND owner_id = ?",
    ...
    def update_plan(self, plan_id: str, plan_version: int, plan: TaskPlan, *, owner_id: str = "") -> dict[str, Any]:
        current = self.get_plan(plan_id, plan_version, owner_id=owner_id)
        ...
        with self._connection() as connection:
            latest_version = connection.execute(
                "SELECT MAX(plan_version) AS version FROM plans WHERE plan_id = ? AND owner_id = ?", (plan_id, owner_id)
            ).fetchone()["version"]
            ...
            connection.execute(
                "INSERT INTO plans(plan_id, plan_version, owner_id, topic, payload_json, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (plan_id, next_version, owner_id, next_plan.topic, self._json(payload), RunStatus.PLANNED.value, created_at),
            )
            self._event(
                connection,
                "plan_version_created",
                {"plan_id": plan_id, "plan_version": next_version},
                owner_id=owner_id,
            )
        return self.get_plan(plan_id, next_version, owner_id=owner_id)

    def confirm_plan(self, plan_id: str, plan_version: int, *, owner_id: str = "") -> dict[str, Any]:
        with self._connection() as connection:
            row = connection.execute(
                "SELECT status FROM plans WHERE plan_id = ? AND plan_version = ? AND owner_id = ?", (plan_id, plan_version, owner_id)
            ).fetchone()
            if row is None:
                raise NotFoundError("plan version not found")
            if row["status"] == RunStatus.CONFIRMED.value:
                return self.get_plan(plan_id, plan_version, owner_id=owner_id)
            if row["status"] != RunStatus.PLANNED.value:
                raise InvalidStateTransitionError("only planned plans can be confirmed")
            connection.execute(
                "UPDATE plans SET status = ? WHERE plan_id = ? AND plan_version = ? AND owner_id = ?",
                (RunStatus.CONFIRMED.value, plan_id, plan_version, owner_id),
            )
            self._event(
                connection,
                "plan_confirmed",
    ...
    def cancel_plan(self, plan_id: str, plan_version: int, *, owner_id: str = "") -> dict[str, Any]:
        with self._connection() as connection:
            row = connection.execute(
                "SELECT status FROM plans WHERE plan_id = ? AND plan_version = ? AND owner_id = ?", (plan_id, plan_version, owner_id)
            ).fetchone()
            ...
            connection.execute(
                "UPDATE plans SET status = ? WHERE plan_id = ? AND plan_version = ? AND owner_id = ?",
                (RunStatus.CANCELLED.value, plan_id, plan_version, owner_id),
            )
            self._event(
                connection,
                "plan_cancelled",
    ...
    def create_run(
        ...
        *,
        ...
        with self._connection() as connection:
            connection.execute(
                "INSERT INTO runs(run_id, thread_id, plan_id, plan_version, owner_id, status, payload_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (run.run_id, run.thread_id, plan.plan_id, plan.plan_version, owner_id, run.status.value, self._json(payload), run.created_at, run.updated_at),
            )
            for task in plan.tasks:
                ...
                connection.execute(
                    "INSERT INTO task_runs(run_id, task_id, attempt, owner_id, payload_json, status, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (run.run_id, task.task_id, 0, owner_id, self._json(task_payload), TaskStatus.CONFIRMED.value, utc_now()),
                )
            self._event(
                connection,
                "run_created",
        ...
        return self.get_run(run.run_id, owner_id=owner_id)

    def _get_run_row(self, run_id: str, *, owner_id: str = "") -> sqlite3.Row:
        with self._connection() as connection:
            row = connection.execute("SELECT * FROM runs WHERE run_id = ? AND owner_id = ?", (run_id, owner_id)).fetchone()
        if row is None:
            raise NotFoundError("run not found")
        return row

    def mark_run_running(self, run_id: str, *, owner_id: str = "") -> dict[str, Any]:
        row = self._get_run_row(run_id, owner_id=owner_id)
        ...
        with self._connection() as connection:
            connection.execute(
                "UPDATE runs SET status = ?, payload_json = ?, updated_at = ? WHERE run_id = ? AND owner_id = ?",
                (RunStatus.RUNNING.value, self._json(payload), payload["updated_at"], run_id, owner_id),
            )
            self._event(connection, "run_started", {}, owner_id=owner_id, run_id=run_id)
        return self.get_run(run_id, owner_id=owner_id)

    def cancel_run(self, run_id: str, *, owner_id: str = "") -> dict[str, Any]:
        row = self._get_run_row(run_id, owner_id=owner_id)
        if row["status"] not in {RunStatus.CONFIRMED.value, RunStatus.RUNNING.value}:
        ...
        with self._connection() as connection:
            connection.execute(
                "UPDATE runs SET status = ?, payload_json = ?, updated_at = ? WHERE run_id = ? AND owner_id = ?",
                (RunStatus.CANCELLED.value, self._json(payload), payload["updated_at"], run_id, owner_id),
            )
            task_rows = connection.execute(
                "SELECT task_id, attempt, payload_json, status FROM task_runs WHERE run_id = ? AND owner_id = ?",
                (run_id, owner_id),
            ).fetchall()
            for task_row in task_rows:
                ...
                connection.execute(
                    ...
                    "WHERE run_id = ? AND task_id = ? AND attempt = ? AND owner_id = ?",
                    (
            ...
            self._event(connection, "run_cancelled", {}, owner_id=owner_id, run_id=run_id)
        return self.get_run(run_id, owner_id=owner_id)

    def _latest_task_payloads(self, run_id: str, *, owner_id: str = "") -> dict[str, dict[str, Any]]:
        with self._connection() as connection:
            rows = connection.execute(
                "SELECT task_id, payload_json FROM task_runs WHERE run_id = ? AND owner_id = ? ORDER BY task_id, attempt DESC",
                (run_id, owner_id),
            ).fetchall()
    ...
    def execution_state(self, run_id: str, *, owner_id: str = "") -> dict[str, Any]:
        run = self.get_run(run_id, owner_id=owner_id)
        latest = self._latest_task_payloads(run_id, owner_id=owner_id)
        ...
        return {
            ...
            "run": run["run"],
            "owner_id": owner_id,
            "document_scope": run["run"].get("document_scope", {}),
            "confirmed_plan": True,
    ...
    def persist_graph_result(self, run_id: str, result: dict[str, Any], *, owner_id: str = "") -> dict[str, Any]:
        ...
        with self._connection() as connection:
            ...
            connection.execute(
                "UPDATE runs SET status = ?, payload_json = ?, updated_at = ? WHERE run_id = ? AND owner_id = ?",
                (graph_run.status.value, self._json(graph_run.model_dump(mode="json")), graph_run.updated_at, run_id, owner_id),
            )
            for task_id, task_result in task_results.items():
                ...
                connection.execute(
                    "INSERT OR REPLACE INTO task_runs(run_id, task_id, attempt, owner_id, payload_json, status, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (run_id, task_id, attempt, owner_id, self._json(task_payload), str(task_result.get("status") or TaskStatus.FAILED.value), utc_now()),
                )
            if report_artifact:
                version = connection.execute(
                    "SELECT COALESCE(MAX(version), 0) + 1 AS version FROM report_versions WHERE run_id = ? AND owner_id = ?",
                    (run_id, owner_id),
                ).fetchone()["version"]
                report_artifact["report_version"] = version
                connection.execute(
                    "INSERT INTO report_versions(report_id, run_id, version, owner_id, payload_json, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                    (report_artifact["report_id"], run_id, version, owner_id, self._json(report_artifact), report_artifact.get("created_at") or utc_now()),
                )
            self._event(connection, "run_finished", {"status": graph_run.status.value}, owner_id=owner_id, run_id=run_id)
        return self.get_run(run_id, owner_id=owner_id)

    def prepare_task_retry(self, run_id: str, task_id: str, *, owner_id: str = "") -> dict[str, Any]:
        run = self.get_run(run_id, owner_id=owner_id)
        latest = self._latest_task_payloads(run_id, owner_id=owner_id).get(task_id)
        if latest is None:
        ...
        with self._connection() as connection:
            connection.execute(
                "UPDATE runs SET status = ?, payload_json = ?, updated_at = ? WHERE run_id = ? AND owner_id = ?",
                (RunStatus.CONFIRMED.value, self._json(payload), payload["updated_at"], run_id, owner_id),
            )
            self._event(
                connection,
                "task_retry_requested",
                {"attempt": int(task_result.get("attempts") or 0) + 1},
                owner_id=owner_id,
                run_id=run_id,
                task_id=task_id,
            )
        state = self.execution_state(run_id, owner_id=owner_id)
        state["retry_task_id"] = task_id
        return state

    def get_run(self, run_id: str, *, owner_id: str = "") -> dict[str, Any]:
        row = self._get_run_row(run_id, owner_id=owner_id)
        plan = self.get_plan(row["plan_id"], row["plan_version"], owner_id=owner_id)
        latest = self._latest_task_payloads(run_id, owner_id=owner_id)
        with self._connection() as connection:
            reports = connection.execute(
                "SELECT payload_json FROM report_versions WHERE run_id = ? AND owner_id = ? ORDER BY version",
                (run_id, owner_id),
            ).fetchall()
        task_runs = []
        for task_id, payload in latest.items():
    ...
    def list_runs(self, *, owner_id: str) -> list[dict[str, Any]]:
        """Return current user's persisted run summaries for the history panel."""

        with self._connection() as connection:
            rows = connection.execute(
                "SELECT run_id, status, payload_json, created_at FROM runs WHERE owner_id = ? ORDER BY created_at DESC",
                (owner_id,),
            ).fetchall()
